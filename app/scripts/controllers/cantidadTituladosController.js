...
/**
 * @ngdoc function
 * @name proyectoSDS.controller:CantidadTituladosCtrl
 * @description
 * # CantidadTituladosCtrl
 * Controller of the proyectoSDS
 */
angular.module('proyectoSDS')

  .controller('CantidadTituladosCtrl', function ($scope,$http) {
    ...
    /**
     * Función encargada de buscar los datos de los titulados dependiendo de los valores que contengan los atributos
    ...
    $scope.crearQuery = function crearQuery(){
      var query = 'PREFIX ex: <http://example.org/>'
        ...
        +'SELECT ?nombreInstitucion ?nombreCarrera ?region ?totalTitulados ?totalTituladosFemeninos ?totalTituladosMasculinos ?horario ?anio {'
        +' ?x rdf:type ex:CantidadTitulados. '
        +' ?x ex:institucion ?i. '
        +' ?x ex:carrera ?c. '
        +' ?x ex:region ?region. ';
      if($scope.regionSelected != 'TODAS') query +=' ?x ex:region \''+$scope.regionSelected+'\'.';
      query +=' ?x ex:totalTitulados ?totalTitulados.'
        +' ?x ex:totalFemeninos ?totalTituladosFemeninos.'
        +' ?x ex:totalMasculinos ?totalTituladosMasculinos.'
        +' ?x ex:anio ?anio.';
      ...
      $http.jsonp("http://localhost:3030/ds/query?query="+encodeURIComponent(query)+"&output=json&callback=JSON_CALLBACK")
        .success(function(response) {
          $scope.cantidadTitulados = response.results.bindings;
        }).error(function(){
